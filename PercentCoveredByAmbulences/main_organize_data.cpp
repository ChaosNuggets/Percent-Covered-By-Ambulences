...
std::vector<int> getDurations(std::ifstream& fin)
{
	...
	while (std::getline(fin, line))
	{
		...
		else if (line.find("ZERO_RESULTS") != std::string::npos)
		{
			durations.push_back(999999);
		}
	}
	return durations;
}
...
bool createTravelTimeFile(const int stationNum, bool firstHalf = true)
{
	...
	if (firstHalf)
	{
		fout.open(writeFilePath.c_str());
	}
	else
	{
		fout.open(writeFilePath.c_str(), std::ios_base::app);
	}
	...
	if (firstHalf) createTravelTimeFile(stationNum, false);
	return true;
}
...
int main()
{
	...
	{
		bool success = createTravelTimeFile(i);
		if (!success)
			return 1;
	}
	std::cout << "Success!";
